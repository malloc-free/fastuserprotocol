...
typedef void UTPOnOverheadProc(void *userdata, int send, size_t count, int type);
...
int UTP_SetSockopt(struct UTPSocket *socket, int opt, int val);
...
int UTP_IsIncomingUTP(UTPGotIncomingConnection *incoming_proc,
					   SendToProc *send_to_proc, void *send_to_userdata,
					   const byte *buffer, size_t len, const struct sockaddr *to, socklen_t tolen);
...
int UTP_HandleICMP(const byte* buffer, size_t len, const struct sockaddr *to, socklen_t tolen);
...
int UTP_Write(struct UTPSocket *socket, size_t count);
